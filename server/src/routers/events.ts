 ...
 * The event store's SQL predicate (`EventStore.hasAutoGeneratedEvents` /
 * `deleteAutoGeneratedEvents`) is pinned equal to this one by a dedicated
 * parity unit test (eventStore.test.ts) — two implementations, one tested
 * meaning. Exported for that test; not otherwise used outside this file. */
