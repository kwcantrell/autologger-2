...
function instructionAggregate(categories: readonly AiGenerationSnapshotCategory[]): {
  ...
  for (const cat of categories) {
    entries += 1;
    if (cat.auto_instruction) bytes += Buffer.byteLength(cat.auto_instruction, 'utf8');
...
eventsRouter.post('/api/sessions/:sessionId/events/generate', async (c) => {
  ...
  const categories = bearing.map(toGenerationSnapshotCategory);
  ...
  try {
    const selectedCategories = filterGenerationCategories(categories, body.selection);
    if ((body.selection?.length ?? 0) > 0 && selectedCategories.length === 0) {
      throw new ApiError(400, EVENT_GENERATE_NO_INSTRUCTIONS_DETAIL);
    }
