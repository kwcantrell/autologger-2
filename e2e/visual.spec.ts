...
async function createSession(page: Page): Promise<void> {
  ...
  // Fixed episode text — the title (show code + episode) appears in shots and
  // the default is derived from the show's next_episode counter. Pin it so the
  // deck/rail title text is identical run to run.
  await page.locator('#ns-episode').fill('VIS01');
