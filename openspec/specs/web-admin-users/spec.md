...
The observable behavior of the `/admin/users` operator page: how a user's team memberships are
rendered from the frozen `GET /api/admin/users` response shape (`studios: [{id, name}]`), which
teams the add-membership control offers, what the membership controls act on, and the regression
coverage that keeps the page honest. The client's response type is verified against a captured
real response under the `web-api-response-conformance` capability; this capability states what
the page must do, not the mechanism that proves the type matches.
