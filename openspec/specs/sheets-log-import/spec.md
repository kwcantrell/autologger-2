# sheets-log-import Specification

## Purpose

Server-side import of episode logs kept in Google Sheets (one tab per episode):
reads a public Sheets workbook, matches tabs to sessions by title, aligns log
rows to the session's transcript with a scored per-seam sync, and creates feed
events at corrected session times — driven from Batch Import's Import Logs
control.
