...
Live-recorder chunk rollover and upload-failure rescue. The recorder never holds more
than the in-progress chunk (plus any rescue-queued chunks) in memory, every upload stays
far below the server's live segment cap, and no upload failure silently discards audio
within the page's lifetime.
