...
New capability: user-authored per-button (and per-dropdown-option) generation
instructions, and a user-initiated AUTO GENERATE run that appends transcript-derived
log events through the locked-down Claude CLI one-shot machinery.
