...
// @autologger/domain and consumed by routers/events.ts) and the event
// store's SQL predicate must agree on every metadata_json value in this
// corpus: two implementations, one tested meaning. (persistence-package-
// extraction task 4.1: this predicate moved from routers/events.ts to
// @autologger/domain, killing the only session -> server production/test
// edge; the parity relationship this comment describes is unchanged.)
