# AGENTS.md — autologger

Guidance for coding agents (Cursor and others) in this repo. Loaded as project context.
...
Claude Code reads **`CLAUDE.md`** instead; keep the two files aligned when process or
invariant guidance changes.

## CodeGraph

When a `.codegraph/` index exists at the repo root, prefer CodeGraph before grep/find or
broad file reads when locating or understanding code:

- **MCP tool** (when available): `codegraph_explore` — relevant symbols' verbatim source
  plus call paths (including dynamic-dispatch hops).
- **Shell** (always works): `npx @colbymchenry/codegraph explore "<symbol or question>"`.

If there is no `.codegraph/` directory, skip CodeGraph — indexing is the operator's choice.
