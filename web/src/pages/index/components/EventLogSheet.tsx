...
function FilterDropdown({
  ...
  const checkedClass = 'aria-checked:!bg-transparent aria-checked:!text-[rgba(248,250,252,0.92)]';
  const label = (checked: boolean, text: string) => (
  ...
  return (
    ...
    >
      ...
      >
        {label(showInternal, 'Show internal events')}
      </PopoverItem>
    </Popover>
  );
}
...
export function EventLogSheet({ sessionId }: Props) {
  ...
  const hasAutoGeneratedEvents = events.some(isAutoGeneratedEvent);
  ...
  const sorted = useMemo(() => {
    const filtered = events.filter(
      (event) =>
        !hiddenCategoryIds.has(event.category) &&
        (showInternal || event.category.toLowerCase() !== 'internal'),
    );
    return doSortEvents(filtered, sortState, status);
  }, [events, hiddenCategoryIds, showInternal, sortState, status]);
  ...
  const generateControl = (
    ...
    >
      <PopoverItem
        onClick={() => {
          setGenerateMenuOpen(false);
          handleAutoGenerate(hasAutoGeneratedEvents ? { regenerate: true } : undefined);
        }}
      >
